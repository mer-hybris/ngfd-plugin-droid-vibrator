...
static void binder_died (GBinderRemoteObject *remote, void *user_data);

static int connect (void)
{
    N_DEBUG (LOG_CAT "connect to vibrator service");
    ...
    if (gbinder_servicemanager_wait(m_service_manager, -1)) {
        ...
        if (m_client) {
            m_callback = gbinder_servicemanager_new_local_object (
                ...
                NULL);
            gbinder_local_object_set_stability(m_callback,
    ...
    N_DEBUG (LOG_CAT "connect failed.");
    return -1;
}
